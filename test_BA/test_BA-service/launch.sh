...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "test_BA-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name test_BA-service apps/test_BA-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app test_BA-service:1.0-SNAPSHOT
    oc expose svc/test_BA-service
else

	echo "Launching the application locally..."
	pattern="test_BA-service"
	files=( $pattern )
	cd ${files[0]}

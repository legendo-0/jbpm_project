...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "test_BA-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name test_BA-service apps/test_BA-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app test_BA-service:1.0-SNAPSHOT
		call oc expose svc/test_BA-service
	) else (
